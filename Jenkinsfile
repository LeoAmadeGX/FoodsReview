	...
	post {
		...
		success {
			emailext to: "Leo_Tsai@systemweb.com.tw",
				...
				body: "${env.JOB_NAME} : ${currentBuild.currentResult}<p />RDAP2019 ${PublishSys} 因 master 上出現以下Commit : <br /><strong style=color:#94C1AE;>${CommitNote}</strong> <br />推送 <p /><br />現在已更新到最新版本，請確認。<p />更多資訊請由此查詢: ${env.BUILD_URL}",
                mimeType: 'text/html'
		}
	}
}